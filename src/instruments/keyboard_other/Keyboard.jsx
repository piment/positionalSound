...
export const Keyboard = forwardRef((props, ref) => {
  ...
  const [keyboardCol, keyboardCircleAlpha] = useTexture([
    '/mics_keys/textures/keyboard_graphics.png',
    '/mics_keys/textures/keysoft-min.png',
  ]);
  keyboardCol.flipY = false;
  const whiteKeysMat = useMemo(
    () =>
  ...
  const circleMat = new THREE.MeshBasicMaterial({
    transparent: true,
    alphaMap: keyboardCircleAlpha,
  });
  return (
    <group
      {...props}
      dispose={null}
      rotation={ props.rotation? props.rotation :   [0, Math.PI * 0.5, 0]}
      position={ props.position ? props.position : [-4, 0, 1]}
    >
      <mesh
        //  Stand
        castShadow
